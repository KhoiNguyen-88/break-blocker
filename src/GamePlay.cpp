...
void GamePlay::Update(Uint32 dt)
{
...
void GamePlay::Render()
{
	...
	SDL_SetRenderDrawColor(Shared::m_renderer, 0, 255, 0, SDL_ALPHA_OPAQUE);
	SDL_RenderFillRect(Shared::m_renderer, &m_button);

	// copy a portion of the texture to the current rendering target.
	bg.w = WIDTH; bg.h = HEIGHT;
	SDL_RenderCopy(Shared::m_renderer, m_background->m_texture, NULL, &bg);
	...
	for (int i = 0; i < NUMBER_OF_TARGET; i++) {
		// draw target
		SDL_SetRenderDrawColor(Shared::m_renderer, 0, 0, 255, SDL_ALPHA_OPAQUE);
		SDL_RenderFillRect(Shared::m_renderer, &gTarget[i]);
...
void GamePlay::CheckgBrickHitRacket() {
...
void GamePlay::CheckGameOver() {
	...
	if (dst.y == HEIGHT - 40)
	{
		...
		printf("%d", Shared::isRunning);
		*/
		system("pause");
	}
}

void GamePlay::Target() {
	for (int i = 0; i < NUMBER_OF_TARGET; i++) {
		if (dst.x < gTarget[i].x + gTarget[i].w &&
			dst.x > gTarget[i].x &&
			dst.y < gTarget[i].y + gTarget[i].h &&
...
void GamePlay::RenderFrame(int frameIndex, int x, int y)
{
	SDL_Rect srcRect;
	...
	SDL_RenderCopy(Shared::m_renderer, m_SpriteFireBall->m_texture, &srcRect, &dstRect);
}

void GamePlay::HandleKeyEvent(int key, bool state)
{
	SDL_Log("Key: %c", key);
	if (key == 'a') {
		gRacket.x = gRacket.x - RACKET_SPEED;
	}
	else if(key == 'd'){
		gRacket.x = gRacket.x + RACKET_SPEED;
	}
